    ...
    <div className="min-h-screen bg-background text-foreground">
      <div className="mx-auto max-w-5xl px-4 py-10">
        ...
        <div className="mt-10 rounded-xl border border-border bg-muted/20 p-5 text-sm text-muted-foreground space-y-2">
          ...
          <p>This portfolio implements the full agent-native lifecycle end to end: <strong>Discover</strong> (llms.txt, entity.json, JSON-LD) → <strong>Authenticate</strong> (auth.md, this demo) → <strong>Authorize</strong> (scoped, short-lived credentials) → <strong>Act</strong> (the authenticated call to <code>/api/mcp-demo</code> above).</p>
          <p>The underlying problem is the same one Google&apos;s <a href="https://docs.cloud.google.com/iam/docs/agent-identity-overview" target="_blank" rel="noopener noreferrer" className="underline hover:text-foreground">Agent Identity</a> model solves at platform scale — giving each agent its own verifiable identity instead of a shared service account — there implemented with SPIFFE-based IDs and mTLS/DPoP rather than this demo&apos;s auth.md flow. See the <Link href="/enterprise-agent-runtime" className="underline hover:text-foreground">Enterprise Agent Runtime</Link> page for the full ecosystem mapping.</p>
