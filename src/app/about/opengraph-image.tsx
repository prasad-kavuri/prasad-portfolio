import { ImageResponse } from 'next/og';

export const runtime = 'edge';
export const alt = 'About Prasad Kavuri — VP / Head of AI Engineering';
export const size = { width: 1200, height: 630 };
export const contentType = 'image/png';

const BG = '#09090b';
const TEXT_PRIMARY = '#fafafa';
const TEXT_MUTED = '#a1a1aa';
const ACCENT = '#6366f1';
const CARD_BG = '#18181b';
const CARD_BORDER = '#27272a';

const proofPoints = [
  { icon: '▸', text: '200+ engineers led across AI platform orgs' },
  { icon: '▸', text: '70%+ AI infrastructure cost reduction' },
  { icon: '▸', text: '13,000+ B2B customers enabled' },
  { icon: '▸', text: '14 production AI demos / platform patterns' },
];

export default function Image() {
  return new ImageResponse(
    (
      <div
        style={{
          width: '100%',
          height: '100%',
          display: 'flex',
          flexDirection: 'column',
          backgroundColor: BG,
          padding: '56px 64px 0 64px',
          fontFamily: 'sans-serif',
        }}
      >
        {/* Top row */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '48px' }}>
          <span style={{ fontSize: 16, color: TEXT_MUTED, letterSpacing: '0.08em' }}>
            PRASADKAVURI.COM
          </span>
          <span style={{
            fontSize: 12, color: ACCENT, letterSpacing: '0.1em',
            border: `1px solid ${ACCENT}`, padding: '4px 14px', borderRadius: 4,
          }}>
            CANONICAL PROFILE
          </span>
        </div>

        {/* Two-column layout */}
        <div style={{ display: 'flex', flex: 1, gap: 56 }}>
          {/* Left: Identity */}
          <div style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
            <div style={{ fontSize: 52, fontWeight: 700, color: TEXT_PRIMARY, lineHeight: 1.1, marginBottom: 12 }}>
              Prasad
              <br />Kavuri
            </div>
            <div style={{ fontSize: 18, color: ACCENT, marginBottom: 8 }}>
              VP / Head of AI Engineering
            </div>
            <div style={{ fontSize: 15, color: TEXT_MUTED, lineHeight: 1.5 }}>
              Production AI platforms · Agentic AI
              {'\n'}AI Governance · FinOps · Enterprise adoption
            </div>
            <div style={{ marginTop: 20, fontSize: 14, color: TEXT_MUTED }}>
              Chicago / Naperville, IL · Remote-open
            </div>
          </div>

          {/* Right: Proof points */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12, width: 480 }}>
            {proofPoints.map((pt) => (
              <div
                key={pt.text}
                style={{
                  display: 'flex',
                  alignItems: 'center',
                  gap: 14,
                  backgroundColor: CARD_BG,
                  border: `1px solid ${CARD_BORDER}`,
                  borderRadius: 10,
                  padding: '14px 20px',
                }}
              >
                <span style={{ fontSize: 16, color: ACCENT }}>{pt.icon}</span>
                <span style={{ fontSize: 15, color: TEXT_PRIMARY }}>{pt.text}</span>
              </div>
            ))}
          </div>
        </div>

        {/* Bottom accent bar */}
        <div style={{ height: 4, backgroundColor: ACCENT, marginTop: 48, marginLeft: -64, width: 1200 }} />
      </div>
    ),
    { width: 1200, height: 630 },
  );
}
