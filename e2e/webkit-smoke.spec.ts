...
test.describe('WebKit smoke', () => {
  test('homepage renders core profile content', async ({ page }) => {
    ...
    await expect(page.getByText(/Head of AI Platform & Agentic Solutions at Zip/i).first()).toBeVisible();
  });

  test('demos index renders production demo catalogue', async ({ page }) => {
    await page.goto('/demos');

    await expect(page.getByRole('heading', { name: /14 Production AI Modules/i })).toBeVisible({ timeout: 10_000 });
