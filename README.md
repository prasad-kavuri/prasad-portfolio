...
| AI Spatial Intelligence & World Generation | Browser-rendered procedural 3D scene preview (Three.js), GLB export, provider adapter (`hyworld` + deterministic `mock` fallback), policy checks, approval gating | Strengthens logistics and location planning with governed, explainable world artifacts ready for simulation and export handoff (desktop-friendly) |
