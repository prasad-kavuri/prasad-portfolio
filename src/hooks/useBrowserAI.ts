...
export function useBrowserAI(requiresWebGPU = false): BrowserAIStatus {
  ...
  useEffect(() => {
    ...
    if (!wasmOK) {
      setStatus({ capability: 'unsupported', shouldAttemptLoad: false,
    ...
    if (isIOS) {
      setStatus({ capability: 'mobile-ios', shouldAttemptLoad: false,
    ...
    if (isAndroid) {
      setStatus({ capability: 'mobile-android', shouldAttemptLoad: false,
    ...
    if (requiresWebGPU && !hasWebGPU) {
      setStatus({ capability: 'no-webgpu', shouldAttemptLoad: false,
    ...
    if (memory !== undefined && memory < 4) {
      setStatus({ capability: 'low-memory', shouldAttemptLoad: false,
    ...
    if (memory !== undefined && memory < 8) {
      setStatus({ capability: 'low-memory', shouldAttemptLoad: true,
    ...
    // All clear
    setStatus({ capability: 'ready', shouldAttemptLoad: true,
