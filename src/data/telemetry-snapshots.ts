...
export const STATUS_SNAPSHOT = {
  ...
  services: [
    ...
    ['Resume Download', 'Operational · PDF export'],
    ['RAG Pipeline', 'Operational · Browser WASM demo'],
    ['AI Spatial Intelligence & World Generation', 'Operational · Governed world generation'],
...
export const GOVERNANCE_SNAPSHOT = {
  ...
  auditLog: [
    { time: '14:07:58', event: 'guardrail.blocked', detail: 'Prompt injection detected — IP redacted', policy: 'prompt-injection-policy', severity: 'warn' },
    { time: '14:05:03', event: 'guardrail.redacted', detail: 'Competitor mention filtered from output', policy: 'competitor-mention-policy', severity: 'info' },
    { time: '14:03:12', event: 'eval.regression', detail: 'Fidelity Δ +0.02 vs baseline — within gate', policy: '—', severity: 'ok' },
    { time: '14:01:44', event: 'rate_limit.triggered', detail: '429 issued to IP hash a93aa730', policy: '—', severity: 'warn' },
    { time: '13:58:31', event: 'deploy.passed', detail: 'CI gate passed — fidelity 0.94, halluc. 0.02', policy: '—', severity: 'ok' },
    { time: '13:55:09', event: 'guardrail.blocked', detail: 'Template injection {{}} in query body', policy: 'prompt-injection-policy', severity: 'warn' },
    { time: '13:51:22', event: 'eval.completed', detail: 'Snapshot run — eval posture recorded', policy: '—', severity: 'ok' },
  ] as const,
  metricSnapshot: {
...
export function getGovernanceMetricsView(
  ...
  return {
    cards: [
      ...
      ['Eval Fidelity Score', evalFidelityValue.toFixed(2), 'Rubric scoring, gate ≥ 0.85', 'ok'],
